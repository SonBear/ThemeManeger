...
public class ThemeManager {
    ...
    public static void applyTheme(ColorPalette colorPalette, Container window) {
        ...
        components.forEach(comp -> {
            if (comp instanceof JPanel) {
                ...
                changeBorder(label.getBorder(), label, colorPalette);
                ...
                changeBorder(text.getBorder(), text, colorPalette);
    ...
    private static void changeBorder(Border border, JLabel label, ColorPalette palette) {
        if (border instanceof TitledBorder) {
            ...
            Border bdr = BorderFactory.createLineBorder(palette.getSpecificColor(ColorKey.COLOR_FOREGROUND), 2, true);
            TitledBorder titledBorder = BorderFactory.createTitledBorder(bdr, tittle);
            titledBorder.setTitleColor(palette.getSpecificColor(ColorKey.COLOR_FOREGROUND));
            label.setBorder(titledBorder);

        }
    }

    private static void changeBorder(Border border, JTextField label, ColorPalette palette) {
        if (border instanceof TitledBorder) {
            String tittle = ((TitledBorder) border).getTitle();
            Border bdr = BorderFactory.createLineBorder(palette.getSpecificColor(ColorKey.COLOR_FOREGROUND), 2, true);
            TitledBorder titledBorder = BorderFactory.createTitledBorder(bdr, tittle);
